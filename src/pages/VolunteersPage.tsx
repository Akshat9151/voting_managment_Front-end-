...
import { UserPlus, Search, Phone, CheckCircle2, PencilLine, Trash2 } from 'lucide-react';
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-2 border-b border-slate-200/80">
        <div>
          <h1 className="text-xl sm:text-2xl font-extrabold font-heading text-slate-900">{t('teamMembersPageTitle')}</h1>
          <p className="text-xs text-slate-500 mt-0.5">{t('teamDescription')}</p>
        </div>
        <div className="flex items-center gap-2">
          <Badge variant="purple" className="px-3 py-1 text-xs">{filteredTeam.length} {t('memberCountLabel')}</Badge>
          {currentRole !== 'VOLUNTEER' && (
            <Button variant="primary" size="sm" onClick={() => setIsAddModalOpen(true)} leftIcon={<UserPlus className="w-4 h-4" />} disabled={isLoading}>
              {isLoading ? t('memberLoading') : t('memberAdd')}
            </Button>
          )}
        </div>
      </div>
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex rounded-xl border border-slate-200 bg-slate-50 p-1 w-full max-w-md overflow-x-auto">
          {(['ALL', 'ADMIN', 'VOLUNTEER'] as const).map((filter) => {
            const isVisible = currentRole === 'SUPER_ADMIN' || filter === 'VOLUNTEER';
            ...
            return (
              <button
                ...
                className={`flex-1 min-w-[90px] rounded-lg px-3 py-2 text-[11px] font-bold transition-all ${
                  memberFilter === filter ? 'bg-white text-slate-900 shadow-sm border border-slate-200' : 'text-slate-500 hover:text-slate-700'
                }`}
              >
            ...
            <Card key={member.id} variant="interactive" className="flex flex-col justify-between space-y-4">
              <div className="space-y-3">
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="font-heading font-extrabold text-sm text-slate-900 leading-tight">{member.name}</h3>
                ...
                <div className="bg-slate-50 rounded-xl p-3 space-y-1.5 text-xs text-slate-600 border border-slate-100">
                  <div className="flex justify-between"><span className="text-slate-400">{t('memberAssignedWard')}:</span><span className="font-bold text-slate-800 text-right">{member.ward}</span></div>
                  <div className="flex justify-between"><span className="text-slate-400">{t('memberVotersHandled')}:</span><span className="font-bold text-sky-600">{(member.votersHandled ?? 0).toLocaleString()} {t('memberElectorsLabel')}</span></div>
                  <div className="flex justify-between"><span className="text-slate-400">{t('memberAddedDate')}:</span><span className="font-medium text-slate-700">{member.addedDate}</span></div>
                </div>
              </div>

              <div className="flex items-center justify-between gap-2 pt-2 border-t border-slate-100">
                <a href={`tel:${member.phone}`} className="inline-flex items-center gap-1.5 text-xs font-bold text-sky-600 hover:text-sky-800">
              ...
              {canRemove && (
                <div className="flex gap-2 pt-2 border-t border-slate-100">
                  <button type="button" onClick={() => handleEditMember(member)} className="flex-1 inline-flex items-center justify-center gap-1.5 rounded-lg border border-slate-200 bg-slate-50 px-2 py-1.5 text-[11px] font-bold text-slate-700">
                    <PencilLine className="w-3.5 h-3.5" /> {t('memberEditAction')}
                  </button>
                  <button type="button" onClick={() => handleRemoveMember(member)} className="flex-1 inline-flex items-center justify-center gap-1.5 rounded-lg border border-rose-200 bg-rose-50 px-2 py-1.5 text-[11px] font-bold text-rose-700">
                    <Trash2 className="w-3.5 h-3.5" /> {t('memberRemoveAction')}
                  </button>
                </div>
              )}
            </Card>
      ...
      <Modal isOpen={isAddModalOpen} onClose={() => setIsAddModalOpen(false)} title={<div className="flex items-center gap-2"><UserPlus className="w-5 h-5 text-sky-600" /><span>{t('addCampaignTeamMember')}</span></div>}>
        <form onSubmit={handleAddMember} className="space-y-4">
          ...
          <div className="flex justify-end gap-2 pt-3">
            ...
            <Button type="submit" variant="primary">{t('memberSave')}</Button>
          </div>
        </form>
      </Modal>
    </div>
